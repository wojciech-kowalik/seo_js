 ...
 * 
 * @class Seo.MonitoringQuestionEdit
 * @constructor 
 * @namespace Seo
 * @author w.kowalik 
 * @access public
...
Seo.MonitoringQuestionEdit = Class.create(Seo.Element.prototype,{
   ...
   setListeners: function () {
    ...
    $('.question-save').click(function() {
       ...
       var min = parseInt($("#configuration_min").val()), 
           max = parseInt($("#configuration_max").val());
       
       if(min >= max){
           Seo.modalBox.error("Wartość minimum jest większa lub równa maksimum");
           return;
       }       
    ...
    $('.question-save-as').click(function() {
        ...
        form.attr('action', Routing.generate('monitoring_question_save_as', {id: $(this).data('id'), layout: $('#layout').data('content')}));
       ...
       $('#form-question-edit').attr('action', $(this).data('href')); 
        ...
        if($('#has-versions').data('content') == 0){   
            ...
            $.ajax({
                type: 'GET',
                dataType: 'json',
                url: Routing.generate('monitoring_question_primary_version', {
...
$(document).ready(function () {
    ...
    Seo.questionEdit = new Seo.MonitoringQuestionEdit();
